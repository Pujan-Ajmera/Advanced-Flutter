...
class QuizPage extends StatelessWidget {
  final String name;
  final bool isMultiTopic;
  QuizPage({super.key, required this.name, this.isMultiTopic = false});
  ...
  Widget build(BuildContext context) {
    // Initialize controller properly for both single and multi-topic quizzes
    if (isMultiTopic) {
      // For multi-topic quizzes, use existing controller with loaded questions
      try {
        controller = Get.find<QuizController>();
      } catch (e) {
        controller = Get.put(QuizController());
      }
      // Start timer directly since questions are already loaded
      controller.startTimer(quizDuration.inSeconds);
    } else {
      // For single-topic quizzes, create new controller and load questions
      controller = Get.put(QuizController());
      controller
          .loadQuestions(name)
